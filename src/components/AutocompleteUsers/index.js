...
import { instancesV2 } from "@src/@core/plugin/axios"
...
const AutocompleteUsers = ({ selectedUser, handleSelectedUser, t, params }) => {
  ...
  const onSearch = async () => {
    ...
    if (query?.trim()) {
      setLoading(true)
      instancesV2
        .get(`admin/user/active/${query}`, { params: params || {} })
        .then((res) => {
          if (res?.data?.data) {
            ...
            <div className="account-info text-align-left">
              <div>
                ...
                <p className="text-secondary d-flex align-items-center m-0">
                  {t("Full name")}: {selectedUser?.full_name}
                </p>
                <p className="text-secondary d-flex align-items-center m-0">
