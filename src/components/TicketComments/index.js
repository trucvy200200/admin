...
import { createUserTicketComment, getUserTicketComments } from "@pages/userTicket/store/action"
import { uploadFile } from "@store/actions/common"

import "react-image-lightbox/style.css"
import "./styles.scss"

const TicketComments = ({ id, status, showComment }) => {
  ...
  const {list: comments, total: totalComments, params: commentsParams} = useSelector((state) => state.userTicket?.ticketComments)
  const [firstTime, setFirstTime] = useState(false)
  const [isLoading, setIsLoading] = useState(false)
  const [isPending, setPending] = useState(false)
  const rowsPerPage = 10
  const [currentPage, setCurrentPage] = useState(+commentsParams?.offset > 0 ? +commentsParams?.offset / 10 : 1)
  ...
  const fetchData = async () => {
    ...
    if (comments?.length !== 0 && comments?.length === totalComments) {
      setHasMore(false)
    } else {
      const result = dispatch(getUserTicketComments(id, setPending, { limit: rowsPerPage, offset: currentPage + 1 === 1 ? 0 : (currentPage + 1 - 1) * rowsPerPage }))
      if (scrollEl) {
  ...
  const getDataTicketComment = async () => {
    if (isPending) return
    setFirstTime(true)
    dispatch(getUserTicketComments(id, setPending, { limit: rowsPerPage, offset: 0 }, true))
    setFirstTime(false)
  }
  ...
  const handleCreateUserTicketComment = async (id, data) => {
    await createUserTicketComment(
      id,
      data,
      (message) => {
        setComment("")
  ...
  const handleSendCmt = async (e) => {
    ...
    if (comment || images?.length > 0) {
      ...
      if (images?.length > 0) {
        ...
        handleCreateUserTicketComment(id, obj).then()
      } else {
        handleCreateUserTicketComment(id, obj).then()
      }
    } else {
  ...
  return (
    <div className="prod_reviews w-100">
      {(showComment && status === TICKET_STATUS.PROCESSING) && (
        <>
      ...
      <div className="mx-1 comment_list_wrapper">
        <PerfectScrollbar
          containerRef={(el) => {
        ...
        >
          {comments?.length > 0 ? comments?.map((item, index) => <CommentCard key={index} {...item} />) : <li>{t("No comment yet")}</li>}
        </PerfectScrollbar>
      </div>
    </div>
  )
