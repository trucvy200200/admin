...
import { createAdminTicketCommentBySystem, getAdminTicketCommentsBySystem } from "@src/pages/adminTicket/store/action"
...
const TicketCommentsAdminSystem = ({ id, status, showComment }) => {
  ...
  const { list: comments, total: totalComment, params: ticketParams } = useSelector((state) => state.adminTicket?.ticketCommentsBySystem)
  ...
  const fetchData = async () => {
    ...
    if (comments?.length !== 0 && comments?.length === totalComment) {
      ...
      const result = await store.dispatch(
        getAdminTicketCommentsBySystem(id, setPending, { limit: rowsPerPage, offset: currentPage + 1 === 1 ? 0 : (currentPage + 1 - 1) * rowsPerPage })
      )
      if (scrollEl) {
  ...
  const getDataTicketComment = () => {
    if (pending) return
    setFirstTime(true)
    store.dispatch(getAdminTicketCommentsBySystem(id, setPending, { limit: rowsPerPage, offset: 0 }, true))
    setFirstTime(false)
  }
  ...
  const handleSubmitTicketComment = async (obj) => {
    await createAdminTicketCommentBySystem(
      id,
      obj,
      () => {
...
export default TicketCommentsAdminSystem
