...
const OrderPieChart = () => {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle tag='h4' className="title">{t("Total orders")}</CardTitle>
        <small className='mt-50 text-white'>{t("Number of orders")}: <b>{store?.total}</b></small>
      </CardHeader>
        ...
        <div className='recharts-wrapper'>
          <ResponsiveContainer height={300}>
            <PieChart height={300}>
              ...
              >
                {data.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.color} label />)}
              </Pie>
            </PieChart>
          </ResponsiveContainer>
        </div>
